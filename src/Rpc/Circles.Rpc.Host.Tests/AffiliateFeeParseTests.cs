...
/// <summary>
/// Pure tests for <see cref="CirclesRpcModule.ParseFee"/> — the defensive parser for the group-profile
/// <c>membershipFee</c> value. Groups publish their own profile documents, so the parser must accept the
/// schema's JSON-number shape, tolerate a JSON-string shape and a stray '%', and reject anything else as
/// "no fee" (null) rather than crash or feed garbage into the 100%-cap sum.
/// </summary>
[TestFixture]
public class AffiliateFeeParseTests
{
