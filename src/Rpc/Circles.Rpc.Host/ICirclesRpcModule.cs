...
public interface ICirclesRpcModule
{
    ...
    /// </summary>
    Task<object> GetTokenBalances(string address);
    ...
    /// </summary>
    Task<object> GetTokenInfo(string tokenAddress);

    /// <summary>
    /// Gets information about multiple tokens by their addresses.
    /// </summary>
    Task<object> GetTokenInfoBatch(string[] tokenAddresses);
    ...
    /// </summary>
    Task<object> GetAvatarInfo(string address);

    /// <summary>
    /// Gets avatar information for multiple addresses.
    /// NOTE: Currently returns V2 avatars only.
    /// </summary>
    Task<object> GetAvatarInfoBatch(string[] addresses);
    ...
    /// </summary>
    Task<object> GetProfileCid(string address);

    /// <summary>
    /// Gets profile CIDs for multiple addresses.
    /// </summary>
    Task<object> GetProfileCidBatch(string[] addresses);

    /// <summary>
    /// Gets a profile by its CID from IPFS storage.
    /// Results are cached in memory.
    /// </summary>
    Task<object> GetProfileByCid(string cid);

    /// <summary>
    /// Gets multiple profiles by their CIDs from IPFS storage.
    /// </summary>
    Task<object> GetProfileByCidBatch(string[] cids);

    /// <summary>
    /// Gets a profile by avatar address.
    /// Looks up the CID first, then retrieves the profile.
    /// </summary>
    Task<object> GetProfileByAddress(string address);

    /// <summary>
    /// Gets multiple profiles by avatar addresses.
    /// </summary>
    Task<object> GetProfileByAddressBatch(string[] addresses);
    ...
    Task<object> SearchProfiles(string text, int limit = 20, int offset = 0, string[]? types = null);
    ...
    /// </summary>
    Task<object> GetTrustRelations(string address);
    ...
    Task<object> GetCommonTrust(string address1, string address2, int? version = null);
    ...
    /// </summary>
    Task<object> Query(SelectDto query);
    ...
    /// </summary>
    Task<object> FindPathV2(FlowRequest flowRequest);

    /// <summary>
    /// Gets a snapshot of the entire Circles trust network.
    /// This method proxies the request to an external Pathfinder service.
    /// </summary>
    Task<object> GetNetworkSnapshot();
    ...
    /// </summary>
    Task<object> GetTables();
}

// ========================================================================
// DTOs (Data Transfer Objects)
// ========================================================================
// Note: These are currently defined but not used in the interface return types
// because the implementation returns dynamic/anonymous objects for JSON-RPC compatibility.
// In a future refactoring, these could replace `object` return types with proper typing.
// ========================================================================
