...
create or replace view public."V_CrcV2_GroupMembersCount_1h"("group","timestamp",value) as
...
changes_hourly as (
    select
        -- bucket to UTC hours, keep timestamptz
        (date_trunc('hour', (to_timestamp(mc.change_epoch) at time zone 'UTC')) at time zone 'UTC')::timestamptz as "timestamp",
        mc."group",
...
calendar as (
    ...
    cross join generate_series(
        r.min_ts_utc,
        (date_trunc('hour', (current_timestamp at time zone 'UTC')) at time zone 'UTC')::timestamptz,
        interval '1 hour'
    ) as gs
),
dense as (
    select c."group", c."timestamp", coalesce(h.delta,0) as delta
    from calendar c
    left join changes_hourly h
      on h."group" = c."group" and h."timestamp" = c."timestamp"
)
