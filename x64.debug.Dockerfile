...
COPY --from=build /circles-nethermind-plugin/Circles.Pathfinder.Host.dll ./artifacts/bin/Nethermind.Runner/debug/plugins/
COPY --from=build /circles-nethermind-plugin/Circles.Pathfinder.Host.pdb ./artifacts/bin/Nethermind.Runner/debug/plugins/
